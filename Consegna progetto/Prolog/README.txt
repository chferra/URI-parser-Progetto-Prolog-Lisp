...
2)	I predicati uri_parse fungono da 'root', cioè contengono tutti i sottogoal che estrapolano dalla stringa "la propria
	parte" e convogliano il resto della stringa scartata dal sottogoal precedente verso quello successivo.

3) 	Tutti i caratteri previsti dalla tabella ASCII (salvo quelli eventualmente non supportati dall'interprete Prolog) 
	sono da considerarsi accettabili. Perciò non sono previsti ulteriori "filtri" sui caratteri ammessi se non quelli 
	specificati dalla traccia.

4)	Il carattere space viene gestito in modo tale che, qualora venisse letto dal predicato che si occupa di gestire i 
	caratteri, questo verrà convertito nella sequenza '%20'.
	
5)	L'interpretazione dei predicati uri_parse è strutturata in modo tale da fornire 2 possibili tipi di risposta a 
...
6)	La componente 'path' nel caso di schema 'zos' è obbligatoria solo se prima viene letto uno '/', che introduce la 
